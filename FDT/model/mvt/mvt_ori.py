...
class MVT_ori(nn.Module):
    def __init__(self, img_size=(240, 320), patch_size=(24, 32), in_chans=1, num_classes=1000, embed_dim=384, depth=12,
        ...
        self.patch_to_embedding = nn.Conv2d(in_channels=1, out_channels=self.embed_dim, kernel_size=patch_size, stride=patch_size)
        ...
        self.blocks1 = Transformer(dim=embed_dim, depth=int(depth-cut), heads=num_heads, mlp_dim=int(mlp_ratio*embed_dim))
    ...
    def forward_features(self, x):
        x = self.patch_to_embedding(x).flatten(2).transpose(1, 2)
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        x = torch.cat((cls_token, x), dim=1)
        x = self.pos_drop(x + self.pos_embed)
        x = self.blocks1(x)
        B, N, C = x.shape
        x = x.reshape(B//3, N*3, C) # there should be change according to the number of views
        x = self.blocks2(x)
        x = self.norm(x)
        return x[:, 0]
